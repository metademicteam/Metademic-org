      ...
      try {
        ...
        if (r.ok) {
          ...
          if (status === "failed" || errStr) {
            ...
            if (out) {
              await supabase.from("racn_jobs").update({ status: "completed", output: out, completed_at: new Date().toISOString() }).eq("id", id);
              ...
              await supabase.from("racn_jobs").update({ status: "completed", completed_at: new Date().toISOString() }).eq("id", id);
              job.status = "completed";
            }
